...
When(
  '{actor} subtracts the second from the first',
...
When(
  '{actor} multiplies the result by {int}',
  async (actorName: string, multiplier: number) => {
    await actorCalled(actorName).attemptsTo(MultiplyResult.by(multiplier));
  },
);

Then(
  '{actor} should see the result is {int}',
  async (actorName: string, expected: number) => {
...
Then(
  '{actor} should know that Actors represent users',
  async (actorName: string) => {
    await actorCalled(actorName).attemptsTo(
      Ensure.that(
...
Then(
  '{actor} should know that Abilities define what Actors can do',
  async (actorName: string) => {
    await actorCalled(actorName).attemptsTo(
      Ensure.that(
        ScreenplayKnowledge.aboutAbilities(),
        equals(
          'Abilities define what an Actor can do (e.g., parse schemas, generate data)',
        ),
      ),
    );
  },
...
Then(
  '{actor} should know that Tasks represent high-level actions',
  async (actorName: string) => {
    await actorCalled(actorName).attemptsTo(
      Ensure.that(
        ScreenplayKnowledge.aboutTasks(),
        equals(
          'Tasks represent high-level business actions composed of Interactions',
        ),
      ),
    );
  },
...
Then(
  '{actor} should know that Questions query system state',
  async (actorName: string) => {
    await actorCalled(actorName).attemptsTo(
      Ensure.that(
