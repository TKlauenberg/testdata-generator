...
- [ ] Implement Xoshiro256** core algorithm (AC: 1, 2, 3, 9, 10)
  - [ ] Create `packages/core/src/generator/rng.ts` file
  - [ ] Define RNG interface with required methods
  - [ ] Implement SplitMix64 seeding algorithm for state initialization
  - [ ] Implement Xoshiro256** state transition with rotl helper
  - [ ] Add state management with BigUint64Array (4x 64-bit values)

- [ ] Implement RNG factory function (AC: 1)
  - [ ] Create `createRNG(seed?: number): RNG` factory
  - [ ] Handle optional seed parameter (generate default if not provided)
  - [ ] Convert number seed to bigint for internal use
  - [ ] Return configured RNG instance

- [ ] Implement core random methods (AC: 2, 3, 4, 5)
  - [ ] Implement `nextInt(): number` returning 32-bit integers
  - [ ] Implement `nextFloat(): number` using upper 53 bits for IEEE 754 precision
  - [ ] Implement `nextIntRange(min: number, max: number): number` with rejection sampling
  - [ ] Implement `nextFloatRange(min: number, max: number): number` for floating ranges
  - [ ] Validate input parameters (min <= max, finite numbers)

- [ ] Export through module index (AC: 11)
  - [ ] Create/update `packages/core/src/generator/index.ts`
  - [ ] Export `createRNG` function
  - [ ] Export `RNG` interface type
  - [ ] Follow project barrel export patterns

- [ ] Write comprehensive unit tests (AC: 12)
  - [ ] Create `packages/core/src/generator/rng.test.ts`
  - [ ] Test: Same seed produces identical sequences (10+ values)
  - [ ] Test: Different seeds produce different sequences
  - [ ] Test: nextFloat() returns values in [0, 1) range
  - [ ] Test: nextIntRange() respects min/max bounds
  - [ ] Test: nextFloatRange() respects min/max bounds
  - [ ] Test: Parameter validation (min > max throws/returns error)
  - [ ] Test: Default seed generation when seed omitted
  - [ ] Test: State transitions follow Xoshiro256** spec

- [ ] Write Gherkin BDD tests (AC: 13)
  - [ ] Create `packages/core/features/prng-determinism.feature`
  - [ ] Scenario: Same seed produces identical random sequences
  - [ ] Scenario: Different seeds produce different sequences  
  - [ ] Scenario: Random floats distributed uniformly in [0, 1)
  - [ ] Scenario: Random integers respect specified ranges
